...
analysis_graph<- function(df, title){
  ...
  return(ggplot(df, aes(x=value, ymin=min_val, y=average_victory_percentage, ymax=max_val, color=average_victory_percentage))+
    geom_pointrange(data=subset(df,variable=="sizeMap"))+
    geom_pointrange(data=subset(df,variable=="sizeCoalition")) +
    geom_pointrange(data=subset(df,variable=="intSolo"))+
    geom_pointrange(data=subset(df,variable=="intCoalition"))+
    geom_pointrange(data=subset(df,variable=="intDiff"))+
...
analyze_data <- function(victory_stats, title){
  ...
  df <- all_melted_data %>% group_by(variable, value) %>% summarize(min_val= min(victory_percent), max_val=max(victory_percent), average_victory_percentage=mean(victory_percent))
...
analyze_binned_data<- function(binned_data, title){
  ...
  df <- all_melted_data %>% group_by(variable, value, bin) %>% summarize(min_val= min(victory_percent), max_val=max(victory_percent), average_victory_percentage=mean(victory_percent))
  return(analysis_graph(df, title)+
