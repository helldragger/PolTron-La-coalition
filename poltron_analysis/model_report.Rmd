...
victory_stats$C <- as.character(victory_stats$C)
victory_stats$Ds <- as.character(victory_stats$Ds)
victory_stats$Dc <- as.character(victory_stats$Dc)
victory_stats$diffD <- as.character(victory_stats$diffD)
...
basic_analysis <- function(victory_stats, title){
  ...
  return(ggplot(dat.long, aes(value,victory_percent, color=victory_percent, fill=victory_percent))+
    ...
    scale_color_gradient(low="gray25", high="gray65")+
...
analyze_binned_data(binned_data, "Analysis per victory percent interval")
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
binned_data <- victory_stats %>% group_by( bin=C )
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
binned_data <- victory_stats %>% group_by(bin=Ds)
...
binned_data <- victory_stats %>% group_by(bin=Dc)
...
analyze_binned_data(binned_data, "Analysis per map size")
```

## Locking Ds-Dc into 5 equally spaced intervals

```{r echo=FALSE, message=FALSE, warning=FALSE}
binned_data <- victory_stats %>% group_by(bin=diffD)
